import { Component, VElement, div, ul, li, commandLink, span, input, isNullOrEmpty, checkbox } from 'pickle-ts'
    ...
    view (remove?: () => void) : VElement {
        return (
            li ({ key: this.name, class: hoverArea + " d-flex" }, 
                closeButton ( {adjustY: -6}, { onclick: remove!, class: showOnHover }),
                checkbox (this, () => this.done, { label:
                    span ({ class: "pl-1"},
                        ! this.done ? this.name : span ({ style: {textDecoration: 'line-through', color: "lightgray"}}, this.name)
                    )
                })
            )
        )       
